...
export async function runSearchSetupFlow(
  ...
  await prompter.note(
    [
      "网页搜索让您的智能体在线查找信息。",
      "选择一个提供商。部分提供商需要 API 密钥，部分无需密钥即可使用。",
      "Docs: https://docs.openclaw.ai/tools/web",
    ].join("\n"),
    "网页搜索",
  );
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "暂时跳过",
        hint: "稍后通过 openclaw configure --section web 配置",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        `${entry.label} 无需 API 密钥即可使用。`,
        "OpenClaw 将启用该插件并将其用作 web_search 提供商。",
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "已启用密钥引用 — OpenClaw 将存储引用而不是 API 密钥.",
        `环境变量：${ref.id}${envAvailable ? "（已检测到）" : ""}。`,
        ...(envAvailable ? [] : [`在网关环境中设置 ${ref.id}。`]),
        "Docs: https://docs.openclaw.ai/tools/web",
      ].join("\n"),
      "网页搜索",
    );
  ...
  const keyInput = await prompter.text({
    message: keyConfigured
      ? `${credentialLabel}（留空保持当前）`
      : envAvailable
        ? `${credentialLabel}（留空使用环境变量）`
        : credentialLabel,
    placeholder: keyConfigured ? "留空保持当前" : entry.placeholder,
  });
  ...
  await prompter.note(
    [
      `未存储 ${credentialLabel} — 在有可用密钥之前 web_search 无法使用.`,
      `在此获取密钥：${entry.signupUrl}`,
      "Docs: https://docs.openclaw.ai/tools/web",
    ].join("\n"),
    "网页搜索",
  );
